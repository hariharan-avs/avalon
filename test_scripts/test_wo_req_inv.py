    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkOrderSubmit" in input_json_str1 and err_cd == 0:
                ...
                if response["error"]["code"] == 2 and "Improper Json" in response["error"]["message"]:
                    err_cd = 0
                else:
                    logger.info("ERROR: Failed at WorkOrderSubmit - Request submitted successfully where expected to fail.")
                    err_cd = 1
