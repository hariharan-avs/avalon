...
def LocalMain(config) :
    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkerRegister" in input_json_str1 :
                ...
                if "error" in response:
                    if response["error"]["code"] == 0:
                        err_cd = 0
                        logger.info("SUCCESS: Worker Register successfully by-passing additional unkown parameters in input")
                    else:
                        err_cd = 1
                        logger.info("ERROR: Worker Register due to additional unkown parameters in input")
                else:
                    err_cd = 1
                    logger.info("ERROR: Worker Register response not in expected format")
...
def processRequest(file_no, uri_client, input_json_str1):
    ...
    signed_input_file = './worker_results/' + output_json_file_name + req_time + str(file_no) + '_request.json'
    ...
    response_output_file = './worker_results/' + output_json_file_name + req_time + str(file_no) + '_response.json'
