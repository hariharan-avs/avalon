    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkOrderSubmit" in input_json_str1 and err_cd == 0:
                ...
                if response["error"]["code"] == 5 or response["error"]["code"] == 8:
                    err_cd = 0
                else:
            ...
            if "WorkOrderGetResult" in input_json_str1 and err_cd == 0:
                ...
                if "result" in response and "workOrderId" in response["result"].keys() and err_cd == 0 and wo_submit == 0:
                ...
                if "result" in response and "workOrderId" in response["result"].keys() and err_cd == 0 and wo_submit > 0:
                    if response["error"]["code"] == 8:
                        logger.info("SUCCESS: WorkOrder Process Failed with Key already exist error as Expected.")
                        err_cd = 0
                    else:
                        err_cd = 1
