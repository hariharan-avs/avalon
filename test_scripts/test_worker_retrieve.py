            ...
            if "WorkerRetrieve" in input_json_str1 and err_cd == 0:
                ...
                if err_cd == 0:
                    ...
                    if "result" in response:
                        if "workerType" in response["result"].keys():
                            worker_obj.load_worker(response)
                            logger.info("SUCCESS: WorkerRetrieve processed successfully.")
                        else:
                            err_cd = 1
                            logger.info("ERROR: WorkerRetrieve failed to obtain valid result.")
                    else:
                        err_cd = 1
                        logger.info("ERROR: WorkerRetrieve failed to fetch result.")
