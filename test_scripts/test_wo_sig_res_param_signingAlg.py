        ...
        for file in sorted(files) :
            ...
            if "WorkerRetrieve" in input_json_str1 and err_cd == 0:
                ...
                if err_cd == 0:
                    file_no += 1
                    response = processRequest(file_no, uri_client, input_json_str1)
                    response["result"]["details"]["signingAlgorithm"] = "SECP128K1"
            ...
            if "WorkOrderSubmit" in input_json_str1 and err_cd == 0:
                ...
                input_json_final["params"]["requesterSignature"] = ""
                input_json_str1 = json.dumps(input_json_final)
                ...
                if response["error"]["code"] == 5:
                    err_cd = 1
                    logger.info("ERROR: Failed at WorkOrderSubmit - submitted successfully where expected to fail")
                else:
                    logger.info("SUCCESS: Failed at WorkOrderSubmit - Request not submitted successfully as Signing algorithm is not supported as Expected.")
                    err_cd = 0
    else :
