        ...
        for file in sorted(files) :
            ...
            if "WorkerRetrieve" in input_json_str1 and err_cd == 0:
                ...
                if "result" in response and "ids" in response["result"].keys() and err_cd == 0:
                        ...
                        input_json_final["params"]["signingAlg"] = ""
            ...
            if "WorkOrderGetResult" in input_json_str1 and err_cd == 0:
                ...
                #input_json_str1 = json.dumps(input_workorder_getresult, separators=(',', ':'))
                #input_json_str1 = input_workorder_getresult.copy()
                input_json_final = json.loads(input_json_str1)
                input_json_final["params"]["workOrderId"] = work_order_id
                input_json_str1 = json.dumps(input_json_final)
                ...
                if "result" in response and "workOrderId" in response["result"].keys() and err_cd == 0:
                    if response["result"]["workOrderId"] == "":
                        logger.info("ERROR: Failed at WorkOrderGetResult - WorkOrder Not processed successfully.")
                        err_cd = 1
                    else:
                        err_cd = 0
                        logger.info("SUCCESS: WorkOrder Processed Successfully.")
...
def processRequest(file_no, uri_client, input_json_str1):
    ...
    signed_input_file = './workorder_results/' + output_json_file_name + req_time + '_' + str(file_no) + '_request.json'
    ...
    response_output_file = './workorder_results/' + output_json_file_name + req_time + '_' + str(file_no) + '_response.json'

    with open(response_output_file,"w") as resp_file:
        json.dump(response, resp_file)
    #
    # if output_json_file_name == 'wo_success':
    #     if "WorkerLookUp" in input_json_str1:
    ...
    #         if "result" in response:
    #             if response["result"]["workerType"] == 1 or 2 or 3:
    #                 result_cd += 1
    #         else:
    #             err_cd += 1
    #
    #     if "WorkerOrderSubmit" in input_json_str1:
    ...
    return response
#
# def test_worker_lookup(input_json_str1, response, err_cd, result_cd):
#
#     try:
#         if response["result"]["totalCount"] != 0:
#             result_cd == 1
#     except:
#         result_cd == 0
#         logger.info("Response not in expected format")
#
#     return result_cd
#
# def test_worker_retrieve(input_json_str1, response, err_cd, result_cd):
#
#     try:
#         if response["result"]["workerType"] == 1 or 2 or 3:
#             result_cd == 1
...
#     return result_cd
#
# def test_worker_order_submit(input_json_str1, response, err_cd, result_cd):
#
#     try:
#         if response["error"]["code"] != 5:
#             result_cd == 1
...
#     return result_cd
#
# def test_worker_order_getresult(input_json_str1, response, err_cd, result_cd):
#
#     try:
#         if response["result"]["workOrderId"] != '':
#             result_cd == 1
