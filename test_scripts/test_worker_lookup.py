            ...
            if "WorkerLookUp" in input_json_str1:
                ...
                if "result" in  response and "totalCount" in response["result"].keys() and "WorkerLookUp" in input_json_str1:
                    if response["result"]["totalCount"] == 0:
                        err_cd = 0
                        logger.info("SUCCESS: WorkerLookUp processed successfully but did not fetch any ids for lookup criteria.")
                    else:
                        err_cd = 0
                        logger.info("SUCCESS: WorkerLookUp processed successfully and obtained ids for lookup criteria.")
                else:
                    logger.info("ERROR: Worker Lookup did not produce result.")
                    err_cd = 1
