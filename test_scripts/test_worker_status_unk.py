...
def LocalMain(config) :
    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkerSetStatus" in input_json_str1 :
                ...
                if "error" in response:
                    if response["error"]["code"] == 0:
                        err_cd = 0
                        logger.info("SUCCESS: Worker Set Status successfull by-passing additional unkown parameters in input")
                    else:
                        err_cd = 1
                        logger.info("ERROR: Worker Set Status failed due to additional unkown parameters in input")
                else:
                    err_cd = 1
                    logger.info("ERROR: Worker Set Status response not in expected format")
