    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkOrderSubmit" in input_json_str1 and err_cd == 0:
                ...
                input_json_str1 = json.dumps(input_json_final)
                #input_json_str1 = json.dumps(input_workorder, separators=(',', ':'))
                #input_json_str1 = input_workorder.copy()
                ...
                input_json_final_2 = json.loads(input_json_str1)

                indata_objects = input_json_final_2["params"]["inData"]
                indata_objects.sort(key=lambda x: x['index'])

                enc_key = hex(random.randint(1, 2**4))

                i = 0
                for item in indata_objects:
                    input_json_final_2["params"]["inData"][i]["encryptedDataEncryptionKey"] = enc_key
                    i = i + 1

                input_json_str1 = json.dumps(input_json_final_2)
            ...
            if "WorkOrderGetResult" in input_json_str1 and err_cd == 0:
                ...
                if "result" in response and "code" in response["result"].keys() and err_cd == 0:
                    if response["result"]["code"] == -8:
                        logger.info("SUCCESS: Failed Code 8 negative Failed at WorkOrderGetResult as expected.")
                        err_cd = 0
                    else:
                        err_cd = 1
                        logger.info("ERROR: WorkOrder Processed Successfully where expected to fail.")
