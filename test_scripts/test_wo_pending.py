    ...
    if input_json_dir:
        ...
        for file in sorted(files) :
            ...
            if "WorkOrderSubmit" in input_json_str1 and err_cd == 0:
                ...
                response_wait_time = (input_json_final["params"]["responseTimeoutMSecs"])/1000
                input_json_str1 = json.dumps(input_json_final)
            ...
            if "WorkOrderGetResult" in input_json_str1 and err_cd == 0:
                ...
                if "WorkOrderGetResult" in input_json_str1 and "result" not in response:
                ...
                if "result" in response and "workOrderId" in response["result"].keys() and err_cd == 0:
                    if response["result"]["workOrderId"] == "":
                        logger.info("ERROR: Failed at WorkOrderGetResult - WorkOrder Not processed in time.")
                        err_cd = 1
                    else:
                        err_cd = 0
