...
def create_work_order_request(input_json_str1, worker_obj):
    """ Function to create work order request. Uses input json file and modifies empty parameters in json file. """
    ...
    return input_json_str1, work_order_id, request_id
...
def sign_work_order_request(input_json_str1, worker_obj, sig_obj, private_key):
    ...
    return input_json_str1, session_iv, encrypted_session_key
...
def create_work_order_get_result(work_order_id, request_id):
    ...
    input_workorder_getresult = input_data = '{"jsonrpc": "2.0","method": "WorkOrderGetResult","id": 11,"params": {"workOrderId": ""}}'
    ...
    while("result" not in response):
        if "error" in response:
            if response["error"]["code"] != 5:
                logger.info("ERROR: Failed at WorkOrderGetResult - Response received with error code.")
                err_cd = 1
                break

        response = workflow.process_request(uri_client, input_json_str1, output_json_file_name)
        time.sleep(3)

    return response

def verify_work_order_signature(response):

    return err_cd

def decrypt_work_order_response(response):

    return decrypted_response

def process_work_order(input_json, input_type, tamper, output_json_file_name, worker_obj, sig_obj, uri_client, private_key, err_cd, check_submit, check_get_result):
    """ Function to process work order and perform basic validation of return code in response. """
    ...
    if err_cd == 0:
        ...
        input_json_str1, work_order_id, request_id = create_work_order_request(input_json_str1, worker_obj)
        # sign work order request
        input_json_str1, session_iv, encrypted_session_key = sign_work_order_request(input_json_str1, worker_obj, sig_obj, private_key)
        ...
        if err_cd == 0:
            ...
            response = process_work_order_get_result(work_order_id, request_id, uri_client)
            ...
            logger.info("ERROR: WorkOrderGetResult not performed as expected response not received for WorkOrderSubmit.")
    else:
        logger.info("ERROR: No Worker Retrieved from system. Unable to proceed to process work order.")

    return err_cd, input_json_str1, response, processing_time, worker_obj, sig_obj, encrypted_session_key
