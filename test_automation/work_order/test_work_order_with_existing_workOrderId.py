...
def test_work_order_with_existing_workOrderId(setup_config):
    ...
    check_get_result_01 = {"result": {"id": 0}}

    # process worker actions
    err_cd, input_json_str1_01, response_01, processing_time, worker_obj, sig_obj, encrypted_session_key = work_order_utility.process_work_order(input_json_01, input_type_01, tamper, output_json_file_name_01, worker_obj, sig_obj, uri_client, private_key, err_cd, check_submit_01, check_get_result_01)
    ...
    check_get_result_02 = {"result": {"skip": True}}

    # process worker actions
    err_cd, input_json_str1_02, response_02, processing_time, worker_obj, sig_obj, encrypted_session_key = work_order_utility.process_work_order(input_json_02, input_type_02, tamper, output_json_file_name_02, worker_obj, sig_obj, uri_client, private_key, err_cd, check_submit_02, check_get_result_02)

    if err_cd == 0:
        logger.info("SUCCESS: Work order submit with existing workOrderId failed as exepcted.")
    else:
        logger.info("ERROR: Work order submit with existing workOrderId did not receive expected response.")
